...
def push(title: str, markdown: str, webhook_url: str = "") -> dict:
    """把一条『今日播报』推到飞书群。返回 {ok, status, resp}。
    webhook_url 可选——传了就覆盖环境变量（多机器人用不同 webhook 时）。"""
    url = (webhook_url or os.environ.get("FEISHU_WEBHOOK_URL") or "").strip()
    if not url:
